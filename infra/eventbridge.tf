# =============================================================================
# EventBridge resources DISABLED — workshop IAM role lacks EventBridge permissions
# (events:PutRule, events:TagResource, etc.)
#
# HRIS sync functionality is preserved via an inline endpoint in the
# individuals-service: POST /api/individuals-service/sync
# This can be triggered via the API Gateway (manually or from the frontend).
# =============================================================================

# resource "aws_cloudwatch_event_rule" "every_five_minutes" {
#   count               = data.aws_caller_identity.this.id != "000000000000" ? 1 : 0
#   name                = "${var.aws_project}-hris-sync-rule-${local.app_id}"
#   description         = "Fires every 2 hours to trigger the HRIS employee sync safety net"
